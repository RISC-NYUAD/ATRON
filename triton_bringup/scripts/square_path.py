...
class SquarePath(Node):
    ...
    def timer_callback(self) -> None:
        ...
        # Waypoints defined in base_link frame:
        #  P1: 5 m in front of base_link
        #  P2: from P1, 5 m to the right
        #  P3: from P2, 5 m back (\"down\" relative to the initial forward direction)
        #  P4: from P3, 4 m to the left -> ends 1 m to the right of the starting point
        waypoints_base = [
            (10.0, 0.0),      # P1
            (10.0, -10.0),     # P2
            (0.0, -10.0),     # P3
            (0.0, -2.0),     # P4 (1 m to the right of start)
        ]
        ...
        self.get_logger().info(
            f'Published {len(pose_array.poses)} waypoints on /waypoints in map frame')
